...
toList Leaf =  []
toList (Node k a left right) = sortOn fst (toList left ++ [(k,a)] ++ toList right)
...
get k tree = lookup k (toList tree)
...
fromList [] = Leaf
fromList [(k,a)] = Node k a Leaf Leaf
fromList (x:xs) = set (fst x) (snd x) (fromList xs)
