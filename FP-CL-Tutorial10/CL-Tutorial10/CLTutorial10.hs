...
ddelta (FSM  qs as ts ss fs ) source  sym = fromList [getThird arrow | arrow <- toList ts, 
                                                                       startPoint <- toList source, 
                                                                       getFirst arrow == startPoint && getSecond arrow == sym]
  where
    getFirst (a, _, _) = a
    getSecond (_, a, _) = a
    getThird (_, _, a) = a
...
dfinal  fsm@(FSM qs as ts ss fs) supers = filterS (\x -> not $ null $ fs /\ x) supers
  
--fromList [ super | super <- toList supers,fs == super ]
